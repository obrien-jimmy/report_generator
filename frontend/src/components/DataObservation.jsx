...
const DataObservation = ({
  ...
  finalThesis,
  methodology,
  onDataObservationComplete,
  autoSave,
  onAutoSaveDraft,
  dataObservationData,
  literatureReviewData // receives: { questions, responses, contextMapData, etc. } - now used for data observation
}) => {
  ...
  // Restore state from saved dataObservationData
  useEffect(() => {
    console.log('🔍 DataObservation useEffect - literatureData:', literatureData);
    ...
    if (literatureData && typeof literatureData === 'object' && Object.keys(literatureData).length > 0) {
      console.log('DataObservation: Restoring saved state from literatureData:', literatureData);
  ...
  if (literatureData.expandedOutlines) setExpandedOutlines(literatureData.expandedOutlines);
      ...
      console.log('⚠️ DataObservation: Received null/empty literatureData, keeping existing state');
    }
  }, [literatureData]);
  ...
  console.log('🔍 literatureData:', literatureData);
    ...
    } else if (outlineData && outlineData.length > 0 && outlineData.some(section => section.is_data_section) && (!questions || Object.keys(questions).length === 0)) {
      console.log('� Extracting questions from outlineData...');
      const extractedQuestions = {};
      
      outlineData.forEach((section, sectionIndex) => {
        console.log(`🔍 Processing section ${sectionIndex}:`, section.section_title, 'is_data_section:', section.is_data_section);
        if (section.is_data_section && section.subsections) {
    ...
    if ((literatureData?.questions || (outlineData && outlineData.length > 0 && outlineData.some(section => section.is_data_section))) && literatureData?.responses) {
      setTimeout(() => {
    ...
    <div className="literature-review">
      ...
      <div className="mb-4">
        <div className="d-flex align-items-center gap-3 mb-3">
          <h3 className="mb-0">Enhanced Data & Observation Builder</h3>
            ...
            {contextMapData ? (
              <div>
                <div className="row mb-3">
                  ...
                  <div className="col-md-6">
                    <h6>Data & Observation Context Analysis</h6>
      ...
      <div className="alert alert-primary mb-4">
        <h6>Enhanced Data & Observation Builder Process</h6>
        <p className="mb-2">
          This enhanced builder provides a systematic three-step approach to data & observation development.
        </p>
        <div className="row">
        ...
        <div className="step-2-interface mb-4">
          ...
          {detailedOutlineComplete && (
            <div className="alert alert-success">
              <strong>✅ Enhanced Data & Observation Complete!</strong> 
